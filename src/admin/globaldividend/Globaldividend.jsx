...
const Globaldividend = () => {
  ...
  // Update percent value (admin editable)
  const handlePercentChange = (index, value) => {
    const updated = [...percentList];
    updated[index].percent = value === "" ? "" : Number(value);
    setPercentList(updated);
  };
  ...
  const handleDistribute = async () => {
    ...
    // Only include valid percentage rows
    const validRanks = percentList.filter(
      (p) => p.percent !== "" && p.percent > 0
    );
            ...
            >
              ...
              <div className="flex items-center">
                <input
                  ...
                  className={`w-16 bg-transparent text-right font-semibold outline-none border-none appearance-none ${item.percent == 0 ? "text-red-600" : "text-blue-600" }`}
                />
                <span
                  className={`ml-1 font-semibold ${item.percent == 0 ? "text-red-600" : "text-blue-600"
                    }`}
                >
                  %
                </span>
